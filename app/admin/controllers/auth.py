...
def login():
    ...
    if totp_mod.is_2fa_enabled(user):
        ...
        resp = make_response(
            render_template("auth/twofa.html", title="Two-Factor Authentication", backup_mode=False)
        )
...
def setup():
    ...
    log_audit(
        ...
        "admin",
        "setup_complete",
        f"email={email},domain={domain_name}",
        request.remote_addr,
...
def _sync_setup_domain_to_mail_api(domain):
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            "mail-api add_domain failed for %s (non-fatal)", domain.name, exc_info=True
        )
...
def _redirect_to_setup():
    if request.path == url_for("auth.setup"):
        return None
    needs_setup = (request.path.startswith("/admin") or request.path == url_for("auth.login")) and (
        User.query.filter_by(role="admin").first() is None
    )
    if needs_setup:
