...
def test_api_auto_save_new_recipient(authed_client, app):
    ...
    try:
        with (
            patch("app.modules.contacts.controllers.api._get_credentials", return_value="pw"),
            patch(
                "app.modules.contacts.controllers.api.carddav.discover_address_book"
            ) as mock_disc,
                ...
                data=json.dumps(
                    {
                        ...
                        "recipients": [{"email": "new@example.com", "name": "New User"}],
                    }
                ),
        ...
        with app.app_context():
            ...
            conn = open_cache(paths["cache"], key)
            rows = conn.execute(
                "SELECT fn, email_work FROM contacts WHERE email_work = ?", ("new@example.com",)
            ).fetchall()
            conn.close()
        assert len(rows) == 1
        ...
        with patch("app.modules.contacts.controllers.api._get_credentials", return_value="pw"):
            resp = client.post(
                ...
                data=json.dumps(
                    {
                        "account_id": account_id,
                        "recipients": [{"email": "existing@example.com", "name": "Existing"}],
                    }
                ),
                content_type="application/json",
            )
...
def test_api_auto_save_no_carddav(authed_client, app):
    ...
    try:
        resp = client.post(
            "/app/contacts/api/auto-save",
            data=json.dumps(
                {"account_id": account_id, "recipients": [{"email": "any@example.com"}]}
            ),
            content_type="application/json",
        )
    ...
    try:
        with (
            ...
            resp = client.post(
                "/app/contacts/api/auto-save",
                data=json.dumps(
                    {"account_id": account_id, "recipients": [{"email": "down@example.com"}]}
                ),
                content_type="application/json",
            )
    ...
    try:
        with (
            ...
            resp = client.post(
                "/app/contacts/api/auto-save",
                data=json.dumps(
                    {"account_id": account_id, "recipients": [{"email": "jane@company.org"}]}
                ),
                content_type="application/json",
            )
        ...
        with app.app_context():
            ...
            conn = open_cache(paths["cache"], key)
            rows = conn.execute(
                "SELECT fn, email_work FROM contacts WHERE email_work = ?", ("jane@company.org",)
            ).fetchall()
            conn.close()
        assert len(rows) == 1
        ...
        with (
            ...
            resp = client.post(
                ...
                data=json.dumps(
                    {
                        "account_id": account_id,
                        "recipients": [{"email": invalid_email, "name": "Bad"}],
                    }
                ),
                content_type="application/json",
            )
    ...
    try:
        with (
            ...
            mock_create.side_effect = [
                ("/abook/uid-1.vcf", "etag-1"),
                ("/abook/uid-2.vcf", "etag-2"),
            ]
...
def test_contact_edit_duplicate_email_rejected(authed_client, app):
    ...
    try:
        with patch("app.modules.contacts.controllers.contacts.carddav"):
            resp = client.post(
                f"/app/contacts/{account_id}/uid-b2/edit",
                data={
                    "fn": "Bob Updated",
                    "first_name": "Bob",
                    "last_name": "Updated",
...
def test_contact_edit_same_email_allowed(authed_client, app):
    ...
    try:
        with patch("app.modules.contacts.controllers.contacts.carddav") as mock_carddav:
            ...
            resp = client.post(
                f"/app/contacts/{account_id}/uid-same/edit",
                data={
