...
export function registerCalendarTools(server: McpServer, client: ApiClient) {
  server.tool(
    "calendar_list_calendars",
    "List available calendars with name, color, and default status",
    {
  ...
  server.tool(
    ...
    async ({ account_id, calendar_id, name, color }) => {
      ...
      const data = await client.put(
        `/api/v1/calendar/calendars/${encodeURIComponent(calendar_id)}`,
        body,
      );
      return json(data);
    },
  ...
  server.tool(
    ...
    async ({ account_id, calendar_id }) => {
      const data = await client.delete(
        `/api/v1/calendar/calendars/${encodeURIComponent(calendar_id)}`,
        { ...client.accountId(account_id), confirm: true } as Record<string, unknown> as Record<string, string> | undefined,
      );
      return json(data);
    },
  );
  ...
  server.tool(
    ...
    "List events in a calendar with date range",
    {
      account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
      calendar_id: z.string().describe("Calendar ID"),
    ...
    async ({ account_id, calendar_id, since, until, search, cursor, max_results }) => {
      const data = await client.get(
        `/api/v1/calendar/calendars/${encodeURIComponent(calendar_id)}/events`,
        {
          ...client.accountId(account_id),
          since,
          until,
  ...
  server.tool(
    "calendar_get_event",
    "Get full event detail including attendees, recurrence, and reminders",
    {
      account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
    ...
    async ({ account_id, event_id }) => {
      const data = await client.get(
        `/api/v1/calendar/events/${encodeURIComponent(event_id)}`,
        client.accountId(account_id),
      );
      return json(data);
    },
  ...
  server.tool(
    ...
    "Search events by summary across calendars",
    {
      account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
  ...
  server.tool(
    ...
    async ({ account_id, event_id, summary, description, location, start, end, is_all_day, calendar_id, attendees, reminders, recurrence, scope }) => {
      ...
      if (is_all_day !== undefined) body.is_all_day = is_all_day;
      if (calendar_id !== undefined) body.calendar_id = calendar_id;
  ...
  server.tool(
    ...
    "Check free/busy time ranges for calendars",
    {
      account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
