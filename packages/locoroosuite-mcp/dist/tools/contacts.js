...
export function registerContactsTools(server, client) {
    server.tool("contacts_list", "List contacts in the user's address book. Read-only: returns contact objects (name, email, phone, organization) without modifying any data.", {
    ...
    server.tool("contacts_get", "Get full contact detail including all vCard fields. Read-only.", {
        account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
        contact_id: z.string().describe("Contact ID"),
    }, async ({ account_id, contact_id }) => {
        const data = await client.get(`/api/v1/contacts/${encodeURIComponent(contact_id)}`, client.accountId(account_id));
        return json(data);
    });
    server.tool("contacts_search", "Search contacts by name, email, or phone. Read-only: returns name and email pairs without modifying any data.", {
        account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
    ...
    server.tool("contacts_create", "Create a new contact", {
        account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
    ...
    server.tool("contacts_update", "Update an existing contact", {
        account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
        contact_id: z.string().describe("Contact ID"),
        ...
        if (fn !== undefined)
            body.fn = fn;
        if (email_work !== undefined)
            body.email_work = email_work;
        if (email_home !== undefined)
            body.email_home = email_home;
        if (phone_work !== undefined)
            body.phone_work = phone_work;
        if (phone_cell !== undefined)
            body.phone_cell = phone_cell;
        if (phone_home !== undefined)
            body.phone_home = phone_home;
        ...
        if (note !== undefined)
            body.note = note;
        const data = await client.put(`/api/v1/contacts/${encodeURIComponent(contact_id)}`, body);
        return json(data);
    });
    server.tool("contacts_delete", "Delete a contact", {
        account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
        contact_id: z.string().describe("Contact ID"),
    }, async ({ account_id, contact_id }) => {
        const data = await client.delete(`/api/v1/contacts/${encodeURIComponent(contact_id)}`, client.accountId(account_id));
        return json(data);
    });
    server.tool("contacts_bulk_delete", "Delete multiple contacts", {
        account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
