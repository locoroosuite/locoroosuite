...
class TestParseFetchItemInternalDate:
    def test_extracts_internal_date(self):
        ...
        raw = b"test body"
        uid, _flags, _body, internal_date = _parse_fetch_item((meta, raw))
        assert uid == "42"
    ...
    def test_no_internal_date_returns_none(self):
        ...
        raw = b"test body"
        _uid, _flags, _body, internal_date = _parse_fetch_item((meta, raw))
    ...
    def test_non_tuple_item_no_internal_date(self):
        meta = b"1 (FLAGS (\\Seen) UID 42)"
        _uid, _flags, _body, internal_date = _parse_fetch_item(meta)
        assert internal_date is None
...
class TestUpsertMessageInternalDateFallback:
    ...
    def test_stores_both_date_ts_and_internal_date_ts(self, tmp_path):
        ...
        upsert_message(
            ...
            False,
            "<m1@t.com>",
            internal_date=datetime(2026, 5, 14, 10, 25, 2, tzinfo=UTC),
        )
        ...
        assert internal_date_ts == int(datetime(2026, 5, 14, 10, 25, 2, tzinfo=UTC).timestamp())
    ...
    def test_internal_date_stored_when_date_header_missing(self, tmp_path):
        ...
        expected = int(datetime(2026, 5, 14, 10, 25, 2, tzinfo=UTC).timestamp())
