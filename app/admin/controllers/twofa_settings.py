...
@admin_bp.route("/settings/security/enable", methods=["POST"])
@require_role("admin")
def twofa_enable():
    user = _current_user()
    if not user:
    ...
    if totp_mod.is_2fa_enabled(user):
        return redirect(url_for("admin.twofa_settings"))
    secret = totp_mod.generate_secret()
    session["_pending_totp_secret"] = secret
    return redirect(url_for("admin.twofa_confirm"))


@admin_bp.route("/settings/security/confirm", methods=["GET", "POST"])
@require_role("admin")
def twofa_confirm():
    user = _current_user()
    if not user:
    ...
    logger.info("2FA enabled for admin user_id=%s", user.id)
    session["_new_backup_codes"] = codes
    return redirect(url_for("admin.twofa_settings"))


@admin_bp.route("/settings/security/qr")
@require_role("admin")
def twofa_qr():
    secret = session.get("_pending_totp_secret")
    if not secret:
...
@admin_bp.route("/settings/security/disable", methods=["POST"])
@require_role("admin")
def twofa_disable():
    user = _current_user()
    if not user:
    ...
    logger.info("2FA disabled for admin user_id=%s", user.id)
    return redirect(url_for("admin.twofa_settings"))


@admin_bp.route("/settings/security/regenerate-codes", methods=["POST"])
@require_role("admin")
...
@admin_bp.route("/settings/security/devices/<int:device_id>/revoke", methods=["POST"])
@require_role("admin")
...
@admin_bp.route("/settings/security/devices/revoke-all", methods=["POST"])
@require_role("admin")
