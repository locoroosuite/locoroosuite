...
export function registerDocsTools(server, client) {
    ...
    server.tool("docs_update_tags", "Add/remove tags on a document, or replace the full tag list with `set` (each tag max 50 chars)", {
        account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
    ...
    server.tool("docs_list_tags", "List the distinct tags in use across the account's active documents (sorted, case-insensitive)", {
        account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
    }, async ({ account_id }) => {
