...
export function registerChatTools(server: McpServer, client: ApiClient) {
  server.tool(
    "chat_list_rooms",
    "List chat rooms and direct messages",
    {
  ...
  server.tool(
    "chat_create_room",
    "Create a chat room (or a direct message with is_direct=true and invite)",
    {
      account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
    ...
    async ({ account_id, name, topic, is_public, is_direct, invite }) => {
      ...
      if (is_direct !== undefined) body.is_direct = is_direct;
  ...
  server.tool(
    ...
    async ({ account_id, room_id, limit }) => {
      const params: Record<string, string> = {
        ...(client.accountId(account_id) as Record<string, string>),
      };
      if (limit !== undefined) params.limit = String(limit);
      const data = await client.get(
        `/api/v1/chat/rooms/${encodeURIComponent(room_id)}/messages`,
        params,
      );
      return json(data);
    },
  ...
  server.tool(
    ...
    async ({ account_id, event_id, body }) => {
      const data = await client.post(
        `/api/v1/chat/messages/${encodeURIComponent(event_id)}/edit`,
        { ...client.accountId(account_id), body },
      );
      return json(data);
    },
  );
  ...
  server.tool(
    ...
    async ({ account_id, event_id }) => {
      const data = await client.post(
        `/api/v1/chat/messages/${encodeURIComponent(event_id)}/redact`,
        client.accountId(account_id),
      );
      return json(data);
    },
  ...
  server.tool(
    ...
    async ({ account_id, event_id, key }) => {
      const data = await client.post(
        `/api/v1/chat/messages/${encodeURIComponent(event_id)}/react`,
        { ...client.accountId(account_id), key },
      );
      return json(data);
    },
  );
  ...
  server.tool(
    "chat_mark_room_read",
    "Mark a chat room read up to the given event",
    {
      account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
      room_id: z.string().describe("Matrix room ID"),
  ...
  server.tool(
    "chat_invite_user",
    "Invite a Matrix user to a chat room",
    {
      account_id: z.string().optional().describe("Account ID (uses default if omitted)"),
      room_id: z.string().describe("Matrix room ID"),
  ...
  server.tool(
    ...
    async ({ account_id, room_id }) => {
      const data = await client.post(
        `/api/v1/chat/rooms/${encodeURIComponent(room_id)}/leave`,
        client.accountId(account_id),
      );
      return json(data);
    },
