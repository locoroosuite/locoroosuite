...
from app.shared.models.core import CustomerAccount, ManagerDomain, User
...
def dashboard():
    ...
    domain_ids = [
        link.domain_id for link in ManagerDomain.query.filter_by(manager_id=manager_id).all()
    ]
    customer_ids = [
        row.customer_id
        for row in CustomerAccount.query.filter(CustomerAccount.domain_id.in_(domain_ids)).all()
    ]
