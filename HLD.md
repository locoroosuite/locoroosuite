...
U13.9 - Supported file types (MVP): ODF text (.odt), ODF spreadsheet (.ods), ODF presentation (.odp). Non-ODF files (e.g., .pdf, .docx, .xlsx, .pptx) are stored in their original format. The user can optionally convert them to ODF via a "Convert to Docs" action, which creates a separate editable ODF document alongside the original.
...
U13.21 - The editor view shows a minimal top bar with: back-to-list link, document name (editable inline), and a dropdown with Download, Rename, and Delete actions.
