...
class TestContactsTable:
    def test_contact_table_has_expected_columns(self, logged_in_page):
        ...
        assert any(t.upper() == "NAME" for t in header_texts), (
            f"Name column not found in {header_texts}"
        )
        assert any(t.upper() == "EMAIL" for t in header_texts), (
            f"Email column not found in {header_texts}"
        )
        assert any(t.upper() == "PHONE" for t in header_texts), (
            f"Phone column not found in {header_texts}"
        )
        assert any(t.upper() == "ORGANIZATION" for t in header_texts), (
            f"Organization column not found in {header_texts}"
        )
