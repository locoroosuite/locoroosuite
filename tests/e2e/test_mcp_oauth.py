...
def _register_oauth_client(flask_url: str) -> str:
    ...
    assert resp.status_code == 201, (
        f"OAuth client registration failed: {resp.status_code} {resp.text}"
    )
    return resp.json()["client_id"]


def _run_oauth_flow(
    flask_url: str, email: str, password: str, client_id: str, scope: str = "mail.read"
):
    s = requests.Session()
    login = s.post(
        f"{flask_url}/app/login", data={"email": email, "password": password}, allow_redirects=True
    )
    ...
    authorize_post = s.post(
        f"{flask_url}/oauth/authorize",
        data={
            "action": "approve",
            "client_id": client_id,
...
def _mcp_tool_call(
    mcp_url: str, access_token: str, tool_name: str, arguments: dict | None = None
) -> dict:
    resp = requests.post(
        f"{mcp_url}/mcp",
        json={
...
class TestOAuthReconnect:
    def test_second_oauth_connection_works(self):
        client_id = _register_oauth_client(FLASK_URL)
        token1 = _run_oauth_flow(
            FLASK_URL, "e2e-test@test.localhost", E2E_DEFAULT_PASSWORD, client_id
        )
