...
class TestProcessIncomingReply:
    def test_reply_accepted(self):
        ...
        try:
            ...
            event_id = cache_db.upsert_event(
                conn, "reply-test-uid@example.com", "/evt.ics", "e1", cal_id, ical
            )
            ...
            assert "ACCEPTED" in unfolded
            assert "NEEDS-ACTION" not in unfolded
        finally:
            conn.close()
            os.unlink(path)
    ...
    def test_reply_declined(self):
        ...
        try:
            ...
            assert "DECLINED" in _unfold(event["raw_ical"])
        finally:
            conn.close()
            os.unlink(path)
    ...
    def test_reply_updates_only_matching_attendee(self):
        ...
        try:
            ...
            assert "ACCEPTED" in unfolded
            charlie_line = next(
                line for line in unfolded.splitlines() if "charlie@example.com" in line
            )
    ...
    def test_reply_updates_raw_ical(self):
        ...
        try:
            ...
            assert "NEEDS-ACTION" in _unfold(original_raw)

            process_incoming_reply(conn, SAMPLE_ICS_REPLY_ACCEPTED, "bob@example.com")

            updated_raw = _unfold(cache_db.get_event(conn, event_id)["raw_ical"])
    ...
    def test_reply_caldav_failure_does_not_block_cache_update(self):
        ...
        try:
            ...
            attendees = json.loads(event["attendees"])
            assert attendees[0]["partstat"] == "ACCEPTED"
            assert "ACCEPTED" in _unfold(event["raw_ical"])
        finally:
            conn.close()
            os.unlink(path)
...
class TestPatchRawIcalAttendee:
    def test_patch_existing_partstat(self):
        ...
        result = _unfold(_patch_raw_ical_attendee(raw, "bob@example.com", "ACCEPTED"))
    ...
    def test_patch_preserves_other_attendees(self):
        ...
        result = _unfold(_patch_raw_ical_attendee(raw, "bob@example.com", "ACCEPTED"))
        assert "PARTSTAT=ACCEPTED" in result
        charlie_line = next(line for line in result.splitlines() if "charlie" in line)
        assert "NEEDS-ACTION" in charlie_line
    ...
    def test_patch_case_insensitive_email(self):
        ...
        raw = "ATTENDEE;CN=Bob;PARTSTAT=NEEDS-ACTION:mailto:Bob@Example.COM\r\n"
        result = _unfold(_patch_raw_ical_attendee(raw, "bob@example.com", "TENTATIVE"))
