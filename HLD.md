...
U4.4 - If IDLE drops or is unsupported, fall back to polling every 60 seconds; interval is configurable per user. Downgrade to polling requires 3 consecutive IDLE handshake failures (a single transient failure retries after 5s); connection errors (server restarts, drops) reconnect forever with capped 300s backoff instead of downgrading. IMAP server keepalives (e.g. Dovecot's `* OK Still here`) are filtered out and must never trigger a sync.
...
U4.7c - In the folder/message list view, when a thread has more than 4 messages, the 2 newest messages are displayed normally and the remaining older messages are collapsed behind a clickable bar (e.g. "▸ 3 older messages"). Clicking the bar toggles the collapsed messages inline with a smooth slide animation. The thread count badge remains visible on the first message regardless of collapse state. To keep the DOM bounded (prevents browser freezes when a single thread contains thousands of messages, e.g. automated alerts/newsletters sharing a subject), at most `MAX_MESSAGES_PER_THREAD` (50) messages per thread are rendered in the folder list; the collapse bar reflects the true older count and any further messages are reachable via an "open thread" link to the conversation view. Older messages that are not rendered are never counted out of the badge.
...
U6.10 - Compose recipient fields (To, Cc, Bcc) use a chips/tags input with autocomplete from the contacts search API (U11.13). Typing 2+ characters triggers a debounced fetch to `/app/contacts/api/search?q=`; matching contacts appear in a dropdown showing name and email; selecting a contact inserts a removable chip. Manual email entry is also supported (Enter, comma, or Tab creates a chip). Typed text that has not been committed to a chip is also committed automatically on input blur and on form submit (Send and Save Draft harvest any raw text in To/Cc/Bcc before validating), so a user who types an address and clicks Send directly never loses the recipient. If the To field is empty on submit, the form shows an explicit inline error message ("Add at least one valid recipient…") in addition to the red field highlight — never a silent color-only block. Invalid typed text is kept in the input (not cleared) so the user can correct it. Email addresses are case-insensitive in practice: every committed chip lowercases the address (display names keep their case), and the server normalizes To/Cc/Bcc addr-specs to lowercase on send and draft save. Clicking a chip copies its email address to the clipboard and shows a brief "Copied!" tooltip. The chips component is a shared JS widget reused across To, Cc, and Bcc fields. Paste support: pasting comma-separated emails automatically creates individual chips.
...
U6.12 - Attachments use staged uploads: each selected/dropped file is uploaded immediately to a staging endpoint (`POST /app/mail/attachments/stage`) with a real per-file XHR progress bar; the Send and Save-Draft actions reference the resulting staged attachment IDs (plus a `compose_session_id` generated client-side for user uploads, or server-side when pre-staging forwarded/draft/undo-send attachments) rather than the final multipart files. Staging is scoped per user and per compose session under `data/mail_attachments/<user_id>/<compose_session_id>/`, cleaned up on successful send, with opportunistic garbage collection of abandoned staging directories older than 24 hours. Staged files can be removed before sending via `DELETE /app/mail/attachments/<file_id>`.
...
U6.15 - Forwarding (Gmail parity): the forward prefill quotes the original message using its sanitized HTML part when present (falling back to escaped plain text), prefixed with a "---------- Forwarded message ---------" header block showing From, Date (converted to the user's timezone), Subject, To, and Cc. The original email's attachments (including inline `Content-ID` images) are automatically re-attached: they are staged server-side into a compose staging session and seeded onto the compose form as removable attachment cards. Inline images keep their original `Content-ID` and are sent as `multipart/related` so `cid:` references render for recipients; in the editor they display as data URLs and are converted back to `cid:` at send time. Remote (`http`/`https`) images are stripped from the quoted HTML per the privacy-first default. Oversized originals are skipped with a visible notice. Replies use the same HTML-first quote (with the "On <date>, <sender> wrote:" summary) but do not attach the original's attachments.
U6.16 - Draft attachment preservation (Gmail parity): both manual save-draft and auto-save embed the currently staged attachments into the draft MIME, so successive saves never silently strip attachments (staging is no longer deleted on manual save; cleanup happens on send and via the 24-hour staging GC). Resuming a draft (`draft_uid`) restores its attachments from the draft MIME back onto the compose form (re-staged server-side, inline images rendered via their content IDs), and undo-send restores the attachments of the cancelled message the same way — replacing the old "Attachments are not restored automatically" notice.
...
U12.30 - Reminders via VALARM: each event can have zero or more reminders. Supported reminder types: DISPLAY (in-app notification) and EMAIL (email sent via SMTP). TRIGGER is relative to DTSTART (default: -PT15M, i.e., 15 minutes before). Additionally, PUSH (device notification via U24.31) is delivered server-side for push-armed users with calendar notifications enabled.
...
U12.36 - For invitation-type .ics (METHOD:REQUEST), the preview card shows response actions: Accept, Tentative, Decline. Selecting a response imports the event into the user's selected calendar (with the attendee's PARTSTAT set accordingly) and sends a METHOD:REPLY email to the organizer. If the event is already in the user's calendar (including auto-imported invitations, U12.39d), the card shows the current response status pill plus a "View in calendar" link, and the response actions update the existing event and send the METHOD:REPLY.
...
U12.39c - "Create event" from email: a "Create event" action is available in the email toolbar's overflow (⋯) menu (alongside mark-as-read, move-to-folder, print, etc.) for non-draft messages. Clicking it opens the calendar event editor (U12.56b) pre-populated with: subject → SUMMARY, plain-text body (truncated to 2000 characters) → DESCRIPTION, sender email → ATTENDEE. The user can edit all fields before saving. The action navigates to `/app/calendar/events/new` with pre-fill query parameters (summary, description, attendee), which redirects to the calendar index where the editor dialog opens pre-filled; no cross-module Python imports.
U12.39d - Auto-Tentative invitations: when a METHOD:REQUEST invitation email for the account user (as attendee) is fetched during IMAP sync, the app automatically imports the event into the user's default (first) calendar with the user's attendee PARTSTAT set to TENTATIVE and RSVP=TRUE, silently — no METHOD:REPLY email is sent until the user explicitly responds via the U12.36 actions. The invite preview card then reflects the current status. If an event with the same UID already exists in any calendar, the automatic import is skipped so an explicit response is never downgraded. If the user has no calendars cached, the import is skipped with a warning log. Only newly-fetched messages are processed; already-cached email is not backfilled.
...
## UI/UX Design (Google Calendar style)

U12.56 - Calendar UI follows a Google Calendar-inspired design on desktop and mobile. Available views: Day, Week, Month, and Schedule on desktop (default Week, persisted per U12.10); Schedule (default on phones), Day, 3-day, and Month on mobile. Events render as rounded chips tinted with the calendar color (not solid blocks); all-day events render as colored pills. The current time is shown as a red dot + horizontal line; today's column/cell is subtly tinted.
U12.56a - Concurrent (overlapping) events render side-by-side in columns with an overlap layout engine; events never visually stack on top of each other.
U12.56b - Event create/edit is a centered dialog on desktop and a bottom sheet on mobile (replacing the full-page form). The full-page event detail view remains as a deep-link target (e.g. "View in calendar" from mail). The editor posts JSON to `/app/calendar/api/events` (create) and `/app/calendar/api/events/<id>` (update), reusing the same validation, CalDAV, and iMIP "send updates to guests?" flows as the previous form.
U12.56c - Clicking an event shows a popup card (desktop) or bottom sheet (mobile) with event details and Edit/Delete actions; the card links to the full detail page.
...
U13.9 - Supported file types (MVP): ODF text (.odt), ODF spreadsheet (.ods), ODF presentation (.odp), ODF drawing (.odg, produced by PDF/image conversion). Non-ODF files (e.g., .pdf, .docx, .xlsx, .pptx) are stored in their original format. Images (.jpg, .jpeg, .png, .gif, .svg, .webp, .bmp) are also supported as non-ODF originals — stored as-is with a `meta.json` sidecar and opened read-only in Collabora's image viewer (the "paperclip" use case: filing and previewing photos such as receipts). The user can optionally convert them to ODF via a "Convert to Docs" action (see U13.39a for the per-source target), which creates a separate editable ODF document alongside the original. No OCR is performed.
...
U13.32 - Upload document: file upload control accepts .odt, .ods, .odp, .docx, .xlsx, .pptx, .pdf, .txt/.md/.html and other pandoc-compatible text formats, and images (.jpg, .jpeg, .png, .gif, .svg, .webp, .bmp). ODF files are stored directly. Pandoc-compatible text formats are converted to ODF on upload. Non-ODF binaries (PDF, Office, images) are stored in their original format with a `meta.json` sidecar; no automatic conversion occurs on upload.
...
U13.33 - Upload creates a new document record. For non-ODF files, the `original_format` field records the source format (e.g., `"pdf"`, `"docx"`, `"jpg"`); `doc_type` is set to the target ODF type that conversion would produce (e.g., `"odt"` for DOCX, `"ods"` for XLSX, `"odg"` for PDF and images). For ODF files, `original_format` is NULL.
...
U13.39a - **Per-source ODF target** — the target ODF type is derived from the source extension via `target_odf_type()` (`app/shared/pandoc_formats.py`), the single source of truth shared by upload and convert: Office text (.docx/.doc/.rtf/.html/.txt/.md/etc.) → `.odt`; spreadsheets (.xlsx/.xls/.csv) → `.ods`; presentations (.pptx/.ppt) → `.odp`. **PDF and images (.jpg/.jpeg/.png/.gif/.svg/.webp/.bmp) → `.odg` (Drawing)** — LibreOffice/Collabora imports PDFs as vector Draw documents (one page = one draw layer) and embeds images on a Draw page; requesting `.odt` makes Collabora reject the save (HTTP 401, `X-ERROR-KIND: savefailed`). The converted PDF/image is a drawing of the rendered pages, not reflowable editable text (no OCR). This mapping is mirrored by the REST API (`POST /api/v1/docs/documents/<id>/convert`) and MCP (`docs_convert_document`) convert paths.
...
U13.42 - The documents table gains an `original_format` column (TEXT, nullable). When non-NULL, the document is a non-ODF original (PDF, DOCX, images, etc.) and should be opened read-only in Collabora.
...
U15.78 - `POST /api/v1/docs/documents/upload` — upload a document. `Content-Type: multipart/form-data` with file field. Accepted types: `.odt`, `.ods`, `.odp`, `.docx`, `.xlsx`, `.pptx`, `.pdf`, pandoc-compatible text formats, and images (`.jpg`, `.jpeg`, `.png`, `.gif`, `.svg`, `.webp`, `.bmp` per U13.9). Pandoc text formats are converted to ODF; non-ODF binaries (PDF, Office, images) are stored in their original format with `doc_type` set to the conversion target from `target_odf_type()` (U13.33/U13.39a). Returns document metadata. Max size: 50 MB (U13.34).
...
UX8 - Mobile readability baseline: on customer-facing screens below the `md` breakpoint (768px), text meets minimum sizes: list subjects >= 15px, secondary metadata (sender/date/snippet) >= 12px, badges/labels >= 11px; email bodies render at 15px (quoted text 14px) inside the sanitized iframe; text inputs render at >= 16px on mobile to prevent iOS focus-zoom; pinch-zoom is never disabled (exception: Collabora editor host pages per U13.20a, where Collabora's internal document zoom replaces browser zoom). The email body wrapper uses rem-based font sizes so it respects the user's browser font-size preference. Desktop sizes are unchanged (complements UX1/UX6/U24.36; a user-facing text-size setting remains out of scope per UX1).
...
U23.1 - The application emits structured logs to stdout (one line per event) with format `%(asctime)s %(levelname)s %(name)s: %(message)s`. The default log level is WARNING and is controlled by the `LOG_LEVEL` env var. Sync lifecycle, request-timing, and SSE-volume signals below are designed to be useful at the default WARNING level; the INFO-level lines aid investigation when an operator raises `LOG_LEVEL=INFO`.

U23.2 - Slow HTTP requests are logged at WARNING (visible at the default level). Any request whose server-side handling exceeds `SLOW_REQUEST_MS` (default 2000 ms) logs `slow request <method> <path> status=<code> duration_ms=<n> user_id=<id> account_id=<id>`. The long-lived SSE stream path (`/events/...`) and static assets are excluded so streaming connections do not produce noise.

U23.3 - IMAP sync lifecycle is logged at INFO with identifiers and durations: per-folder `imap folder synced account_id=... folder=... path=initial|incremental total=... added=... unread=... duration_ms=...`, and an overall `imap sync complete account_id=... total_cached=... duration_ms=...`. The initial-vs-incremental path is recorded so a runaway first-sync backfill (e.g. after a UIDVALIDITY change or cache reset) is identifiable.

U23.4 - The per-user SSE event queue is bounded (`MAX_QUEUE_SIZE`, default 200). When a connected client cannot drain events fast enough (the signature of the browser-freeze incident), the oldest event is dropped and a WARNING `sse event queue overflow user_id=... event_type=... size=... dropped_type=...` is logged. This both prevents unbounded memory growth and makes a stuck consumer visible in the default-level logs.
...
U24.19 - VAPID keys are auto-generated on first use and persisted in the main app database, with env overrides (`PUSH_VAPID_PUBLIC_KEY` + `PUSH_VAPID_PRIVATE_KEY` pair and `PUSH_VAPID_SUBJECT`) for production deployments. Setting only one of the key pair is a fail-early configuration error. `PUSH_VAPID_SUBJECT` must be a `mailto:` URL or a bare email address (auto-normalized to `mailto:`); anything else is a fail-early configuration error (no silent no-op), since an invalid subject would otherwise break every push send inside the VAPID signing step.
...
## PWA – Notification Categories, Enablement & Headless Delivery

U24.27 - Per-category notification preferences: `CustomerSettings` gains `notify_mail_enabled` (default on) and `notify_calendar_enabled` (default off). The push sender is a category-aware hub: before sending, it checks the per-user toggle for the notification's category (`mail`, `calendar`, or `test`). Category toggles are web-UI-only (Settings → Notifications), stored in the main app database, and saved via the per-control auto-save endpoint (U9.8). Chat/docs categories are future additions.

U24.28 - Persistent enablement UX: the post-install notifications prompt (U24.25) is no longer once-ever. Dismissal (the ✕ button) hides it for 7 days, after which it may be shown again; a "Don't ask again" action opts out permanently (localStorage). The prompt appears in standalone mode and in mobile browsers, whenever notifications are supported, permission is not yet granted/denied, and this device is not yet subscribed. Hard-denied permission never re-prompts — Settings → Notifications keeps showing browser-specific remediation guidance (U24.22). Settings → Notifications also gains a "Send test notification" action (`POST /app/mail/push/test`) that delivers a benign notification to every active device of the user so they can verify the chain end-to-end; with no registered device it returns a structured error pointing at the enable toggle.

U24.29 - Headless push arming (restart-safe, closed-app delivery): a user with at least one active push subscription is "push-armed". For push-armed users the server persists the user's DEK in a `push_key_store` row (`customer_id`, `wrapped_dek`), wrapped with a server-held secret stored on disk (`data/push_keywrap.key`, auto-generated `0600`; env overrides `PUSH_KEYWRAP_SECRET` for the secret — a valid urlsafe-base64 32-byte key, invalid values are a fail-early config error — and `PUSH_KEYWRAP_KEY_PATH` for the file). Deliberate privacy tradeoff, accepted per-product: while notifications are enabled, the server can autonomously decrypt that user's caches and mail credentials without a login session — the same trust it already has while they are logged in. Removing the last subscription/device deletes the row. The wrapped DEK is refreshed on login (DEK rotation after password change) and on subscribe.

U24.30 - On startup and on subscribe/login, the sync manager arms IMAP IDLE (or the polling fallback) for **all** active mail accounts of push-armed users — not just the browser-selected account — and enqueues one incremental INBOX sync per account so new mail in any account generates a push (U24.17 unchanged for the payload). Logging out of a web session does not disarm push for users whose key is available from the push key store (otherwise logging out on desktop would kill phone notifications); disarming happens by removing all subscriptions/devices or disabling notifications.

U24.31 - Calendar reminders via push: a `CalendarReminderWorker` (server-side, ~30s tick) serves push-armed users with `notify_calendar_enabled`. Per account it keeps the encrypted calendar cache fresh via periodic headless CalDAV sync (every 15 minutes), then scans `calendar_events` ⋈ `calendar_reminders`: for each event whose reminder fire time (`dtstart` + VALARM `TRIGGER` duration, e.g. `-PT15M`) falls within the late-firing window (up to 2 hours late), it sends one push (title = event summary, body = start time formatted in the user's timezone per UX3a, click-through `/app/calendar/`) unless already recorded in the `push_calendar_fired` dedup table (`account_id`, `event_uid`, `trigger_val`, unique; rows older than 30 days are pruned). Event TZIDs are resolved per U8.9 (IANA → Windows map → embedded VTIMEZONE → logged-warning UTC fallback). MVP limitation: recurring events (RRULE) fire reminders for the master instance's `dtstart` only; server-side recurrence expansion is out of scope for now. This adds `PUSH` as a third delivery action for U12.30 reminders (DISPLAY and EMAIL unchanged).
...
U25.2 - Chat data is stored in the user's individual SQLCipher database (per-user, same as mail/contacts/calendar), encrypted at rest using the same per-user key derived from their password (M6/M6a). Cached tables: Matrix credentials, rooms, room members, message timeline, read state, sync state.
U25.3 - Chat module lives at URL prefix `/app/chat/` with its own full MVC stack under `app/modules/chat/`.
U25.4 - Homeserver is configured per-domain by the admin, same pattern as IMAP/SMTP/CardDAV/CalDAV (U1.3/U12.5): `matrix_host`, `matrix_port`, `matrix_use_tls`, `matrix_shared_secret` (for shared-secret account provisioning). Dev target is a Synapse container (`dev-infra/synapse`, SQLite backend, internal network only). Production already runs Synapse as a container (U25.16); wiring the app to it is an ops task, not a migration.
U25.5 - Chat is tied 1:1 to mail accounts. The account switcher is reused. The Matrix user localpart equals the mail account username/email localpart; the server name comes from the domain's homeserver. The mapping is deterministic and transparent: the UI shows it (e.g. `test4@test.localhost` → `@test4:locoroo.test`) when starting conversations and in DM headers; DM creation also accepts exact email addresses and resolves them server-side.
U25.6 - Account provisioning: the Matrix account is created automatically when an administrator creates a suite account (all creation modes) and when a signup invite is completed — a random password is set, the display name becomes the email localpart, and the password backup is stored on the account row encrypted with a key derived from the domain's registration shared secret (so user password changes cannot orphan it). Lazy provisioning on first chat open remains only as a fallback for pre-existing accounts. The per-user encrypted cache holds the access token, device id, and a user-key-encrypted password copy; on invalid tokens the module re-logs in from either backup.
U25.7 - Real-time: incremental Matrix `/sync` long-polling is driven by the chat SSE stream (same SSE infrastructure as mail at `/events/stream`). Sync results update the cache, then are pushed to the browser as `chat_*` SSE events. Multiple tabs share per-user dedup: one sync loop per user session. A headless background sync worker (unread updates with no tab open) is phase 2 (U25.55).
U25.8 - Rooms: create public/private rooms (name, topic), invite by exact email address (the invitee's chat identity is provisioned automatically if missing; raw Matrix user ids also work via the API), join (by id/alias), leave, forget. Direct messages are created by entering the peer's exact email address — no directory search in the UI; user-directory search remains a REST API capability. DMs are Matrix rooms created with `is_direct`, listed in a separate DM group, and show an empty-timeline welcome state naming the peer and their chat identity. Room list shows unread notification counts (from sync) and favorite pinning is phase 2. User discovery APIs require the homeserver to enable `user_directory: search_all_users: true` (set in the dev Synapse seed; must also be set in production), because Synapse otherwise only returns users sharing a room with the searcher. Conversations across homeservers (users whose domain maps to a different server) are rejected with a clear error.
U25.9 - Messages: send text (plain + formatted body). Upload images/video/audio/files via the Matrix media repository (`mxc://` URIs). All media uploads and downloads are proxied through authenticated module endpoints — the browser never talks to Synapse directly (privacy: no third-party origin, media access requires the user session).
U25.10 - Message actions: edit (Matrix `m.replace` with `m.new_content`; cache stores the latest content), delete (redaction; cached copy is redacted), reactions (Matrix `m.annotation`; toggle by emoji, counts shown).
U25.11 - Read state: read receipts (`m.read`) and fully-read markers per room; unread badges reset when a room is opened. Typing notifications (`m.typing`). Presence display is best-effort.
U25.12 - Search: MVP filters rooms and the open room timeline client-side; server-side room message search is phase 2.
U25.13 - UI: Slack-inspired Tailwind layout — left sidebar with room/DM list and unread badges, room header (name, topic, members), message timeline with sender grouping and day dividers, composer with attachment button and drag-and-drop. The module switcher in the header gains "Chat".
U25.14 - Parity: REST API under `/api/v1/chat/*` and MCP tools (list/create rooms, list/send/edit/redact messages, reactions, read markers, join/leave/invite, media download). Media upload via REST API/MCP is phase 2 (U25.56). Scope names: `chat:read`, `chat:write`.
U25.15 - Admin: domain Matrix config UI (host/port/TLS/shared secret) alongside existing DAV config, with health check. The chat health check distinguishes three failure states: not configured, unreachable (homeserver cannot be reached), and `auth_backend_down` — the homeserver is up but its delegated authentication backend (MAS) is down. Synapse signals the latter as HTTP 503 `Unable to introspect the access token` on authenticated endpoints while unauthenticated endpoints stay healthy; the Matrix client maps it to `MATRIX_AUTH_BACKEND_DOWN` with an operator-facing message ("the chat server's authentication backend is down; contact the homeserver operator") instead of a generic error.
U25.16 - Production already runs Matrix as containers (`matrix-synapse` from matrixdotorg/synapse, plus Matrix Authentication Service), PostgreSQL-backed and effectively greenfield (facts re-verified 2026-09). Its config is the homeserver container's mounted `homeserver.yaml` (the host path `/etc/matrix-synapse/conf.d/` is a stale leftover from a previous deployment and is not read). The app container reaches the host-networked Synapse via the compose network gateway IP — requiring a dedicated Synapse listener bound to that gateway (port 8008, TLS off) plus a firewall allow rule for the compose subnet; the gateway IP depends on the compose network's subnet. Integrating chat means wiring the domain config to it (host/port, `registration_shared_secret`) and ensuring `user_directory: search_all_users: true` (required for DM/invite user discovery), not migrating data — see `docs/chat-prod-integration.md`. Caveats: if Synapse delegates login to MAS, the module's password re-login fallback is unavailable (access tokens are long-lived, so this only matters after token revocation); image tags must be pinned rather than `:latest` (the weekly `update-matrix.timer` pulling `:latest` should be replaced), the chat module verified against Synapse 1.159+.

## Out of Scope for MVP

U25.50 - End-to-end encryption (Olm/Megolm) — deferred. Transport security is TLS; trust model is the self-hosted server (same as mail). True E2EE would require a browser-direct client architecture, which is a separate future decision.
U25.51 - Federation — the homeserver is configured internal-only; no server-to-server federation for MVP.
U25.52 - Voice/video calls.
U25.53 - Threads (Slack-style reply threads) — phase 2.
...
U26.10 - i18n is a requirement for every user-facing change, new features and bug fixes alike: user-visible strings go through `_()` / `N_()` (Python) or `window.LR.t()` (JS), and date/time/number formatting goes through Flask-Babel (server, U26.7) or `Intl` (client). Client-side `Intl` APIs require BCP-47 locale tags (`es-ES`); the JS catalog (`window.LR_I18N.locale`) exposes gettext catalog names (`es_ES`, U26.2), so client code must normalize underscores to hyphens before any `Intl` call and fall back to English if `Intl` rejects the tag. Changes that touch user-facing UI or strings are verified in both supported locales (an unlocalized string or an unnormalized locale crashing the view JS is a bug, not a cosmetic issue).
