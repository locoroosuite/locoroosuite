...
class TestApiTokenAuth:
    def test_missing_auth_header(self, app, api_customer):
        client, _user_id, _account_id = api_customer
    ...
    def test_invalid_token_value(self, app, api_customer):
        client, _user_id, _account_id = api_customer
        resp = client.get(
            "/api/v1/accounts", headers={"Authorization": "Bearer lr_invalidtoken123"}
        )
        data = json.loads(resp.data)
        assert resp.status_code == 401
        assert data["error"]["code"] == "AUTH_INVALID"
...
class TestApiTokenScopeEnforcement:
    ...
    def test_missing_scope_denied(self, app, api_customer):
        ...
        resp = client.get(
            f"/api/v1/mail/folders?account_id={account_id}", headers=_make_auth_header(token_value)
        )
...
class TestTokenManagement:
    ...
    def test_revoke_token(self, app, api_customer):
        ...
        resp = client.delete(f"/api/v1/tokens/{token_id}", headers=_make_auth_header(token_value))
    ...
    def test_revoke_nonexistent_token(self, app, api_customer):
        ...
        with app.app_context():
            token_value, _ = _create_token(app, user_id)
        resp = client.delete("/api/v1/tokens/9999", headers=_make_auth_header(token_value))
...
class TestTokenService:
    ...
    def test_wrap_unwrap_roundtrip(self):
        from app.api.token_service import unwrap_dek_from_token, wrap_dek_with_token
    ...
    def test_create_and_authenticate(self, app, api_customer):
        _client, user_id, _account_id = api_customer
        with app.app_context():
            token_value, _token_obj = _create_token(app, user_id, name="auth-test")
    ...
    def test_authenticate_invalid_token(self, app, api_customer):
        with app.app_context():
            ...
            token, _ctx = authenticate_token("lr_nonexistent")
            assert token is None

    def test_dek_credential_wrap_unwrap(self):
        from app.api.token_service import unwrap_dek_from_credential, wrap_dek_with_credential
...
class TestApiSettingsUI:
    ...
    def test_create_token_via_settings(self, app, api_customer):
        ...
        resp = client.post(
            "/app/mail/settings/api/tokens/create",
            data={
    ...
    def test_revoke_token_via_settings(self, app, api_customer):
        ...
        with app.app_context():
            _token_value, token_obj = _create_token(app, user_id, name="revoke-me")
            token_id = token_obj.id
...
class TestApiSettingsLink:
    def test_settings_page_has_api_link(self, app, api_customer):
        ...
        assert resp.status_code == 200
        assert b"/app/mail/settings/api" in resp.data
