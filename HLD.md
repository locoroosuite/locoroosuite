...
U25.15 - Admin: domain Matrix config UI (host/port/TLS/shared secret) alongside existing DAV config, with health check.
U25.16 - Production already runs Matrix as containers (`matrix-synapse` from matrixdotorg/synapse, plus Matrix Authentication Service), SQLite-backed and effectively greenfield (as inspected 2026-08). Integrating chat means wiring the domain config to it (host/port, `registration_shared_secret`), not migrating data — see `docs/chat-prod-integration.md`. Caveats: if Synapse delegates login to MAS, the module's password re-login fallback is unavailable (access tokens are long-lived, so this only matters after token revocation); the weekly `update-matrix.timer` pulling `:latest` should be replaced with a pinned, tested version (the chat module is verified against Synapse 1.159).
